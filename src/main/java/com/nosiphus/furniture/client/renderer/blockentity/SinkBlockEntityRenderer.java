...
public class SinkBlockEntityRenderer implements BlockEntityRenderer<SinkBlockEntity> {
    ...
    public void render(SinkBlockEntity tileEntity, float partialTicks, PoseStack poseStack, MultiBufferSource source, int light, int overlay) {
        ...
        poseStack.translate(-0.5, -0.5, -0.5);
        this.drawFluid(tileEntity, poseStack, source, 2/16F, 12/16F, 2/16F, 12/16F, 3/16F, 12/16F, light);
        poseStack.popPose();
    }
    ...
    private void drawFluid(SinkBlockEntity te, PoseStack poseStack, MultiBufferSource source, float x, float y, float z, float width, float height, float depth, int light) {
        ...
        if (fluid != Fluids.EMPTY && te.getLevel() != null) {
            ...
            float fillLevel = (float) te.getTank().getFluidInTank(0).getAmount() / te.getTank().getCapacity();
