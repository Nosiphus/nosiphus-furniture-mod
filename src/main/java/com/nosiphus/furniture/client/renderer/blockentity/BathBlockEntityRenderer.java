...
public class BathBlockEntityRenderer implements BlockEntityRenderer<BathBlockEntity> {
    ...
    public void render(BathBlockEntity tileEntity, float partialTicks, PoseStack poseStack, MultiBufferSource source, int light, int overlay) {
        ...
        poseStack.translate(-0.5, -0.5, -0.5);
        this.drawFluidBack(tileEntity, poseStack, source, 2.0F * 0.0625F, 2.0F * 0.0625F, 2.0F * 0.0625F, 14.0F * 0.0625F, 11.2F * 0.0625F, 12.0F * 0.0625F, light);
        this.drawFluidHead(tileEntity, poseStack, source, 16.0F * 0.0625F, 2.0F * 0.0625F, 2.0F * 0.0625F, 14.0F * 0.0625F, 11.2F * 0.0625F, 12.0F * 0.0625F, light);
        poseStack.popPose();
    }

    private void drawFluidHead(BathBlockEntity te, PoseStack poseStack, MultiBufferSource source, float x, float y, float z, float width, float height, float depth, int light) {
    ...
    private void drawFluidBack(BathBlockEntity te, PoseStack poseStack, MultiBufferSource source, float x, float y, float z, float width, float height, float depth, int light) {
        FluidStack fluidStack = te.getTank().getFluid();
        Fluid fluid = fluidStack.getFluid();
        if (fluid != Fluids.EMPTY) {
            IClientFluidTypeExtensions fluidType = IClientFluidTypeExtensions.of(fluid);
            ...
            float vShift = 2.0F / 16.0F * vSpan;
            float minV = sprite.getV0() + vShift;
            ...
            height *= (float) te.getTank().getFluidAmount() / (float) te.getTank().getCapacity();
            VertexConsumer consumer = source.getBuffer(RenderType.translucent());
            Matrix4f matrix = poseStack.last().pose();
